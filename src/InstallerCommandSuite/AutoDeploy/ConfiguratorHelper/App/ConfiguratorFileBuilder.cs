    ...
    public class ConfiguratorFileBuilder
    {
        public static List<string> CreateConfiguratorFile_Classic(Options options)
        {
            string classic = "\"ConfiguratorAutomator.exe\"" + " http://"
                ...
                + options.applicationPool + "&newAppPoolName=" + options.newAppPoolName + "&CBauth=cb_only&IISannon=on&usernamedefault=on&connectCreate=connect&DBserver="
        ...
        public static List<string> CreateConfiguratorFile_Agent(Options options)
        {
            string agent = "\"ConfiguratorAutomator.exe\"" + " http://"
                ...
                + options.applicationPool + "&newAppPoolName=" + options.newAppPoolName + "&IUSR_username=IUSR&IUSR_password=&IISbasic=on&theFunction=Edit&vdtype=&admin_action=1&agent=true&rpf=&docMapper=";
